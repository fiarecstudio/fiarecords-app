    ...
    try {
        ...
        const htmlContent = `
            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd; border-radius: 10px; max-width: 600px; margin: auto;">
                ...
                <p style="color: #666; font-size: 14px;">Gracias por tu confianza.</p>
            </div>
        `;
        
        await enviarNotificacion(email, "Comprobante de Pago", htmlContent, proyecto.empresaId);
...
// ==================================================================
// NUEVA RUTA: NOTIFICACIÓN MANUAL A ARTISTA (EMAIL/WHATSAPP)
// Multi-empresa: Usa configuración de la empresa para el remitente
// ==================================================================
router.post('/:id/notificar', async (req, res) => {
    try {
        ...
        console.error('[POST /:id/notificar] Error:', e);
        res.status(500).json({ error: e.message });
    }
});
