...
// Buscar el patrón exacto de líneas consecutivas
...
if (searchPattern.test(content)) {
    ...
    console.log('Patrón no encontrado con regex. Intentando búsqueda alternativa...');
    
    // Intentar encontrar la posición exacta
    const idx = content.indexOf('if (items.length === 0) {\n    async function reimprimirRecibo');
    console.log('Index con LF:', idx);
    
    const idx2 = content.indexOf('if (items.length === 0) {\r\n    async function reimprimirRecibo');
    console.log('Index con CRLF:', idx2);
    
    if (idx >= 0 || idx2 >= 0) {
        const actualIdx = idx >= 0 ? idx : idx2;
        console.log('Contexto alrededor del índice:', content.substring(actualIdx - 10, actualIdx + 100));
    }
}
