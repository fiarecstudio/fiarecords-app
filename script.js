document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderPaginatedList(endpoint, filterText = null) { 
        ...
        if (currentFilter) { 
            filteredData = data.filter(item => { const name = item.nombre || item.username || item.nombreArtistico || ''; return name.toLowerCase().includes(currentFilter); }); 
        }
        ...
        for (const endpoint of endpoints) { 
            ...
            try { 
                ...
                listEl.innerHTML = data.length ? data.map(item => { 
                    ...
                    return `<li class="list-group-item d-flex justify-content-between align-items-center"><span>${escapeHTML(displayName)}</span><div class="btn-group"><button class="btn btn-sm btn-outline-success" onclick="app.restaurarItem('${item._id}', '${endpoint}')"><i class="bi bi-arrow-counterclockwise"></i></button><button class="btn btn-sm btn-danger" onclick="app.eliminarPermanente('${item._id}', '${endpoint}')"><i class="bi bi-x-octagon-fill"></i></button></div></li>`; 
    ...
    async function saveItem(e, type) { 
        ...
        try { 
            ...
            limpiarForm(form.id); localCache[type] = []; renderPaginatedList(type); 
        } catch (error) { showToast(`Error: ${error.message}`, 'error'); } 
    }
            ...
            if (DOMElements.appLogo && DOMElements.logoInput) { 
                ...
                DOMElements.logoInput.onchange = async (event) => { 
                    ...
                    try { 
                        ...
                        if(config.logoBase64) { DOMElements.appLogo.src = config.logoBase64; } 
