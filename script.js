document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==================================================================
    // 5. GOOGLE DRIVE (LÓGICA MAESTRA -> ARTISTA -> PROYECTO)
    // ==================================================================
    ...
    // ==================================================================
    // 6. DASHBOARD SEGURO (Oculta dinero si no es Admin)
    // ==================================================================
    async function cargarDashboard() { 
        try { 
            ...
            if (stats.showFinancials === false) {
                ...
                kpiIngresos.textContent = `$${(stats.ingresosMes || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}`;
    ...
    async function verificarDisponibilidad() {
        ...
        if(fechaInput._flatpickr && fechaInput._flatpickr.selectedDates[0]) {
             // Ajuste zona horaria local para enviar string correcto YYYY-MM-DD
        ...
        try {
            // La API ahora devuelve ARRAY DE STRINGS DISPONIBLES (ej: ["10:00", "11:00"])
            // Ya filtrado por día cerrado o citas ocupadas.
            const horariosDisponibles = await fetchAPI(`/api/proyectos/disponibilidad?fecha=${fecha}`);
            
            horaSelect.innerHTML = ''; // Limpiar

            if (horariosDisponibles.length === 0) {
                // Si el array está vacío, es que el día está cerrado o todo lleno
                ...
                // Llenar el select con lo que mandó el servidor
    ...
    function filtrarFlujo(filtro) { document.querySelectorAll('#filtrosFlujo button').forEach(b => b.classList.remove('active', 'btn-primary')); const activeBtn = Array.from(document.querySelectorAll('#filtrosFlujo button')).find(b => b.textContent === filtro); if (activeBtn) { activeBtn.classList.add('active', 'btn-primary'); } document.querySelectorAll('.kanban-column').forEach(c => c.style.display = (filtro === 'Todos' || c.dataset.columna === filtro) ? 'flex' : 'none'); procesos.forEach(col => { if (document.getElementById(`columna-${col}`)) document.getElementById(`columna-${col}`).innerHTML = '' }); if (localCache.proyectos) { localCache.proyectos.filter(p => p.proceso !== 'Completo' && p.proceso !== 'Solicitud' && p.estatus !== 'Cancelado' && p.estatus !== 'Cotizacion' && !p.deleted).forEach(p => { const colEl = document.getElementById(`columna-${p.proceso}`); if (!colEl) return; const card = document.createElement('div'); card.className = `project-card`; card.dataset.id = p._id; card.style.borderLeftColor = `var(--proceso-${p.proceso.replace(/\s+/g, '')})`; const serviciosHtml = p.items.length > 0 ? p.items.map(i => `<li class="small">${escapeHTML(i.nombre)}</li>`).join('') : `<li>${escapeHTML(p.nombreProyecto || 'Sin servicios')}</li>`; const artistaNombre = p.artista ? (p.artista.nombreArtistico || p.artista.nombre) : 'Público General'; card.innerHTML = `<div class="project-card-header d-flex justify-content-between align-items-center mb-2"><strong class="text-primary ${p.artista ? 'clickable-artist' : ''}" ${p.artista ? `ondblclick="app.irAVistaArtista('${p.artista._id}', '${escapeHTML(p.artista.nombre)}', '')"` : ''}>${escapeHTML(p.nombreProyecto || artistaNombre)}</strong><select onchange="app.cambiarProceso('${p._id}', this.value)" class="form-select form-select-sm" style="width: auto;">${procesos.filter(pr => pr !== 'Solicitud').map(proc => `<option value="${proc}" ${p.proceso === proc ? 'selected' : ''}>${proc}</option>`).join('')}</select></div><div class="project-card-body"><div class="small text-muted mb-2">🗓️ ${new Date(p.fecha).toLocaleDateString()}</div><ul class="list-unstyled mb-0 small">${serviciosHtml}</ul></div><div class="project-card-footer"><strong class="text-success">$${p.total.toFixed(2)}</strong><div class="btn-group"><button class="btn btn-sm btn-outline-primary" title="Pago" onclick="app.registrarPago('${p._id}')"><i class="bi bi-currency-dollar"></i></button><button class="btn btn-sm btn-outline-secondary" title="Editar" onclick="app.editarInfoProyecto('${p._id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger" title="Borrar" onclick="app.eliminarProyecto('${p._id}')"><i class="bi bi-trash"></i></button></div></div>`; colEl.appendChild(card); }); } }
    ...
    async function cargarHistorial() { const tablaBody = document.getElementById('tablaHistorialBody'); tablaBody.innerHTML = `<tr><td colspan="5">Cargando historial...</td></tr>`; try { historialCacheados = await fetchAPI('/api/proyectos/completos'); tablaBody.innerHTML = historialCacheados.length ? historialCacheados.map(p => { const artistaNombre = p.artista ? (p.artista.nombreArtistico || p.artista.nombre) : 'Público General'; return `<tr><td data-label="Artista" class="${p.artista ? 'clickable-artist' : ''}" ondblclick="app.irAVistaArtista('${p.artista ? p.artista._id : ''}', '${escapeHTML(artistaNombre)}', '')">${escapeHTML(artistaNombre)}</td><td data-label="Total">$${p.total.toFixed(2)}</td><td data-label="Pagado">$${(p.montoPagado || 0).toFixed(2)}</td><td data-label="Fecha">${new Date(p.fecha).toLocaleDateString()}</td><td data-label="Acciones" class="table-actions"><button class="btn btn-sm btn-outline-primary" title="Entrega / Drive" onclick="app.openDeliveryModal('${p._id}', '${escapeHTML(artistaNombre)}', '${escapeHTML(p.nombreProyecto || 'Proyecto')}')"><i class="bi bi-cloud-arrow-up"></i></button><button class="btn btn-sm btn-outline-info" onclick="app.registrarPago('${p._id}', true)" title="Pagos"><i class="bi bi-cash-stack"></i></button><button class="btn btn-sm btn-outline-danger" onclick="app.eliminarProyecto('${p._id}')" title="Borrar"><i class="bi bi-trash"></i></button></td></tr>`; }).join('') : `<tr><td colspan="5" class="text-center">No hay proyectos.</td></tr>`; } catch (error) { tablaBody.innerHTML = `<tr><td colspan="5" class="text-center text-danger">Error al cargar historial.</td></tr>`; } }
    ...
    async function mostrarSeccion(id, updateHistory = true) { 
        ...
        // Control de botón global back si existiera en el HTML (opcional)
    ...
    // --- VISTA ARTISTA (CON EL BOTÓN VOLVER INCRUSTADO) ---
    async function mostrarVistaArtista(artistaId, nombre, nombreArtistico) {
        ...
        try {
            ...
            // Aquí se inserta el botón Volver
    ...
    // Función de PDF corregida (No deforma el logo)
    ...
    async function cargarConfiguracion() { 
        try { 
            ...
            // Cargar imágenes y banco (código existente)...
            ...
            // --- NUEVO: CARGAR TABLA DE HORARIOS ---
            ...
            DIAS_SEMANA.forEach((nombreDia, index) => {
                // index coincida con getDay() de JS (0=Domingo, 1=Lunes...)
    ...
    function setupCustomization(payload) { if (payload.role === 'admin') { if (DOMElements.appLogo && DOMElements.logoInput) { DOMElements.appLogo.style.cursor = 'pointer'; DOMElements.appLogo.title = 'Haz clic para cambiar el logo'; DOMElements.appLogo.onclick = () => DOMElements.logoInput.click(); DOMElements.logoInput.onchange = async (event) => { const file = event.target.files[0]; if (!file) return; const formData = new FormData(); formData.append('logoFile', file); try { await fetchAPI('/api/configuracion/upload-logo', { method: 'POST', body: formData, isFormData: true }); showToast('Logo actualizado!', 'success'); await loadInitialConfig(); const config = await fetchAPI('/api/configuracion'); if(config.logoBase64) { DOMElements.appLogo.src = config.logoBase64; } } catch (e) { showToast(`Error al subir logo`, 'error'); } }; } } }
